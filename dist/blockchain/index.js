...
export class BlockchainService {
    static async initialize() {
        try {
            ...
            // Initialize wallet
            this.wallet = new ethers.Wallet(process.env.PRIVATE_KEY || ethers.Wallet.createRandom().privateKey, this.provider);
            // Initialize verifier contract
            this.verifierContract = new ethers.Contract(process.env.VERIFIER_CONTRACT_ADDRESS, [
    ...
    static async getVerifierContract() {
        if (!this.verifierContract) {
            await this.initialize();
        }
        return this.verifierContract;
    }
    static async verifyProofOnChain(proofId, verification) {
        try {
            ...
            throw new AppError('Blockchain verification failed', 500, 'BLOCKCHAIN_VERIFICATION_FAILED');
        }
    }
    static async verifyBatchProofOnChain(batchId, batchProof) {
        try {
            const batchHash = ethers.id(batchId);
            const encodedBatchProof = ethers.AbiCoder.defaultAbiCoder().encode(['bytes[]'], [batchProof.map((proof) => ethers.toUtf8Bytes(JSON.stringify(proof)))]);
