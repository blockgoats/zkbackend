...
export class SessionService {
    static async createSession(userId, token) {
        ...
        `,
            ...
            // Cache session
            await CacheService.set(`session:${userId}`, session, this.SESSION_CACHE_TTL);
    ...
    static async validateSession(userId, token) {
        try {
            // Check cache first
            const cachedSession = await CacheService.get(`session:${userId}`);
        ...
        `,
            ...
            if (isValid) {
                // Update cache
                await CacheService.set(`session:${userId}`, result.rows[0], this.SESSION_CACHE_TTL);
            }
            return isValid;
        }
        catch (error) {
...
SessionService.SESSION_CACHE_TTL = 3600; // 1 hour
